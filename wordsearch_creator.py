...
class Creator:
    ...
    def __add_word(self, word: str) -> None:
        ...
        assert (isinstance(word, str)), "str expected, got int"
        word = word.upper()

        # Directions for the words to be incremented by
        x_changes, y_changes = [0, 1, 1, 1, 0, -1, -1, -1], [-1, -1, 0, 1, 1, 1, 0, -1]

        # Continuously loops until word can be placed
        while True:
            count, dir_ = 0, randint(0, 7)
        ...
        for _ in range(len(word) - 1):
            pos.append([cur_pos[0] + x_changes[dir_], cur_pos[1] + y_changes[dir_]])
            cur_pos = (cur_pos[0] + x_changes[dir_], cur_pos[1] + y_changes[dir_])
        self.__words_solved[word] = pos
